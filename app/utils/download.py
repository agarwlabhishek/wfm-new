...
def html_download_button(tab, html_buffer, fname, label=None):
    # Prepare download button for exporting the plot as an HTML file
    tab.download_button(
        label=label if label else 'Export Plot (.html)',
...
def csv_download_button(tab, data, fname, label=None):
    # Prepare download button for exporting the plot as an CSV file
    tab.download_button(
        label=label if label else "Export Data (.csv)",
        data=data.to_csv(),
        # Format the file name with the current date-time
